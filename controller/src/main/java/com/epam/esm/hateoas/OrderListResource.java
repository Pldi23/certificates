...
public class OrderListResource extends ResourceSupport {
    ...
    public OrderListResource(List<OrderResource> orders, int pageCurrent, long pageLast, int pageSize, boolean isSelf) {
        this.orders = orders;
        add(linkTo(OrderController.class).withRel(LinkConstant.CREATE_REL).withType(LinkConstant.POST_METHOD));
        if (isSelf) {
            add(Paginator.buildPaginationLinks(EndPointConstant.ORDER_ENDPOINT + "/self", pageCurrent, pageLast, pageSize));
